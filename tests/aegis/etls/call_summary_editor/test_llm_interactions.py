...
class TestGenerateResearchPlan:
    ...
    async def test_happy_path(
        ...
        with patch(
            "aegis.etls.call_summary_editor.main.complete_with_tools",
    ...
    async def test_retries_on_parse_error(self, mock_context, research_prompts):
        ...
        with patch("aegis.etls.call_summary_editor.main.complete_with_tools", mock_llm):
            result = await _generate_research_plan(
                context=mock_context,
                research_prompts=research_prompts,
    ...
    async def test_retries_on_transport_error_with_backoff(
        ...
        with (
            patch("aegis.etls.call_summary_editor.main.complete_with_tools", mock_llm),
            patch("aegis.etls.call_summary_editor.main.asyncio.sleep", new_callable=AsyncMock) as mock_sleep,
        ):
            result = await _generate_research_plan(
                context=mock_context,
                research_prompts=research_prompts,
...
class TestExtractSingleCategory:
    ...
    async def test_fallback_when_not_in_research_plan(
        ...
        assert "No research plan available" in system_msg
    ...
    async def test_raises_on_exhausted_retries(
        ...
        with (
            patch("aegis.etls.call_summary_editor.main.complete_with_tools", mock_llm),
            patch("aegis.etls.call_summary_editor.main.asyncio.sleep", new_callable=AsyncMock),
        ):
