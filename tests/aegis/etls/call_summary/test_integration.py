...
def _setup_mocks():
    ...
    patches["nas_conn"] = patch(
        "aegis.etls.call_summary.main.get_nas_connection",
        return_value=object(),
    )
    patches["find_xml"] = patch(
        "aegis.etls.call_summary.main.find_transcript_xml",
        return_value=MOCK_XML_RESULT,
    )
    patches["parse_xml"] = patch(
        "aegis.etls.call_summary.main.parse_transcript_xml",
        return_value=MOCK_PARSED_TRANSCRIPT,
    )
    patches["extract_blocks"] = patch(
        "aegis.etls.call_summary.main.extract_raw_blocks",
    ...
    patches["bank_data"] = patch(
        "aegis.etls.call_summary.main.build_interactive_bank_data",
        new_callable=AsyncMock,
        return_value=MOCK_BANK_DATA,
    )
    patches["headlines"] = patch(
        "aegis.etls.call_summary.main.generate_bucket_headlines",
        new_callable=AsyncMock,
        return_value={"bucket_0": "Revenue growth remains strong"},
    )
    patches["config_review"] = patch(
        "aegis.etls.call_summary.main.analyze_config_coverage",
        new_callable=AsyncMock,
        return_value={"config_change_proposals": []},
    )
    patches["save"] = patch(
        "aegis.etls.call_summary.main._save_interactive_report_to_database",
        new_callable=AsyncMock,
        return_value=None,
    )
    patches["save_docx"] = patch(
        "aegis.etls.call_summary.main._save_legacy_docx_report_to_database",
        new_callable=AsyncMock,
        return_value=None,
    )
    return patches
