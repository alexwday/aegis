...
from aegis.etls.cm_readthrough.interactive_pipeline import _speaker_role
from aegis.etls.cm_readthrough.interactive_pipeline import (
    QAQuestion,
    _call_tool,
...
from aegis.etls.cm_readthrough.nas_source import (
    extract_raw_blocks,
...
def test_extract_raw_blocks_preserves_participant_type() -> None:
    ...
    <transcript>
      <meta>
        ...
        <participants>
          <participant id="p1" name="Jane Executive" type="corporate"
            title="Group Head, Capital Markets" affiliation="Royal Bank of Canada" />
          <participant id="p2" name="John Analyst" type="analyst"
            title="Managing Director" affiliation="Example Securities" />
        </participants>
      </meta>
      <body>
        <section name="Q&amp;A">
          <speaker id="p2" type="q">
            <plist><p>Can you discuss advisory pipelines?</p></plist>
          </speaker>
...
async def test_call_tool_forces_named_tool_choice() -> None:
    ...
    with patch(
        "aegis.etls.cm_readthrough.interactive_pipeline.complete_with_tools",
...
async def test_generate_section_subtitle_retries_and_normalizes_prefix() -> None:
    ...
    with patch(
        "aegis.etls.cm_readthrough.interactive_pipeline._call_tool",
        new=mock_call_tool,
    ):
