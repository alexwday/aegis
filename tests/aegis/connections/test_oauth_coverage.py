...
class TestOAuthCoverage:
    ...
    @pytest.mark.asyncio
    async def test_oauth_returns_none_token(self, mock_logger, mock_get_token):
        ...
        result = await setup_authentication(execution_id="test-none", ssl_config=ssl_config)
    ...
    @patch("aegis.connections.oauth_connector.get_logger")
    @pytest.mark.asyncio
    async def test_oauth_returns_token_without_access_token(self, mock_logger, mock_get_token):
        ...
        # Test
        ssl_config = {"verify": True, "cert_path": None}
        result = await setup_authentication(execution_id="test-no-access", ssl_config=ssl_config)
    ...
    @patch("aegis.connections.oauth_connector.get_logger")
    @pytest.mark.asyncio
    async def test_setup_authentication_unexpected_exception(self, mock_logger, mock_handle_oauth):
        ...
        # Test
        ssl_config = {"verify": True, "cert_path": None}
        result = await setup_authentication(execution_id="test-unexpected", ssl_config=ssl_config)
    ...
    @pytest.mark.asyncio
    @patch("aegis.connections.oauth_connector.httpx.AsyncClient")
    @patch("aegis.connections.oauth_connector.asyncio.sleep")
    async def test_get_oauth_token_with_5xx_retry(self, mock_sleep, mock_httpx_class):
        """Test get_oauth_token with 5xx error and exponential backoff retry (lines 98-106)."""
        from aegis.connections.oauth_connector import get_oauth_token
        import httpx

        # Setup config
        config.oauth.endpoint = "https://test.com/oauth/token"
        ...
        # Setup mock client that returns 500 then 503 then success
        mock_client = AsyncMock()

        # Create mock responses
        error_response_500 = Mock()
        error_response_500.status_code = 500
        error_response_503 = Mock()
        ...
        success_response.raise_for_status = Mock()  # No error

        # Setup sequence of responses
        mock_client.post = AsyncMock(side_effect=[
            httpx.HTTPStatusError("Server Error", request=None, response=error_response_500),
            httpx.HTTPStatusError("Service Unavailable", request=None, response=error_response_503),
        ...
        result = await get_oauth_token("test-exec-id", ssl_config)
        ...
        # Verify we got the token eventually
