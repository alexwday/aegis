...
def mock_engine():
    ...
    Returns:
        Mock engine object
    """
    with patch("aegis.connections.postgres_connector._engine") as mock:
        yield mock
...
class TestConnectionManagement:
    ...
    def test_get_connection_success(self, mock_engine):
        """Test successful connection retrieval from pool."""
        mock_conn = MagicMock()
        mock_engine.connect.return_value = mock_conn

        with patch("aegis.connections.postgres_connector._get_engine") as mock_get:
            mock_get.return_value = mock_engine

            with get_connection("test-exec-id") as conn:
                assert conn == mock_conn
                mock_engine.connect.assert_called_once()

            mock_conn.close.assert_called_once()

    def test_get_connection_error(self, mock_engine):
        ...
        with patch("aegis.connections.postgres_connector._get_engine") as mock_get:
            ...
            with pytest.raises(SQLAlchemyError, match="Connection failed"):
                with get_connection("test-exec-id"):
                    pass

    def test_close_all_connections(self):
        """Test closing all connections in the pool."""
        with patch("aegis.connections.postgres_connector._engine") as mock_engine:
    ...
    def test_close_all_connections_no_engine(self):
        """Test closing connections when no engine exists."""
        with patch("aegis.connections.postgres_connector._engine", None):
            # Should not raise error
            close_all_connections()

    def test_get_connection_with_exception_in_context(self, mock_engine):
