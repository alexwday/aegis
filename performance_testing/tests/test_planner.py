#!/usr/bin/env python
"""
Performance testing script for the Planner agent.

This script loads and tests the planner agent in the same way that main.py does,
using the same SSL, OAuth, and configuration setup. The planner takes clarifier
outputs and determines which databases to query.
"""
...
sys.path.insert(0, str(Path(__file__).parent.parent))  # For performance_tester
...
from src.aegis.model.agents.planner import plan_database_queries  # noqa: E402
...
def setup_planner_context():
    """
    Set up the planner context exactly as main.py does.
    ...
    logger.info("test_planner.context_setup.started", execution_id=execution_id)

    # Setup SSL configuration
    ssl_config = setup_ssl()
    logger.info(
        "test_planner.ssl_setup.completed",
    ...
    logger.info(
        "test_planner.auth_setup.completed",
        execution_id=execution_id,
    ...
    logger.info(
        "test_planner.database_setup.completed",
        execution_id=execution_id,
    ...
    logger.info("test_planner.context_setup.completed", execution_id=execution_id)

    return context


def test_planner_with_scenario(scenario, context, model_tier=None):
    """
    Test the planner with a single scenario.
    
    The planner agent requires clarifier outputs (banks and periods) in addition
    to the conversation history and latest message.

    Args:
        scenario: Scenario object to test
        context: Planner context from setup_planner_context()
    ...
    # Extract the latest message and conversation from the scenario
    messages = scenario.get_messages()
    query = messages[-1]["content"] if messages else ""
    
    # Get clarifier outputs from scenario expected field
    # We store them in expected since Scenario class doesn't have metadata field
    banks = scenario.expected.get("clarifier_banks", {})
    periods = scenario.expected.get("clarifier_periods", {})
    
    # Create a wrapper function that matches the expected signature
    def planner_wrapper(conversation_history, latest_message, context):
        # Update context with model tier if specified
        if model_tier:
            context = {**context, "model_tier_override": model_tier}
            
        return plan_database_queries(
    ...
    result = run_agent_test(planner_wrapper, scenario, context, model_tier)

    logger.info(
        "test_planner.scenario.completed",
        execution_id=context["execution_id"],
        scenario_id=scenario.id,
...
def main():
    """
    Main entry point for planner performance testing.

    Loads scenarios from planner_scenarios.yaml, runs all tests with small,
    ...
    print("\n" + "=" * 60)
    print("PLANNER AGENT MULTI-MODEL PERFORMANCE TESTING")
    print("=" * 60 + "\n")

    # Load scenarios
    scenario_file = Path(__file__).parent.parent / "scenarios" / "planner_scenarios.yaml"
    ...
    # Setup planner context once (same for all tests)
    print("Setting up planner context (SSL, Auth, Databases)...")
    try:
        context = setup_planner_context()
        print("✓ Planner context initialized")
        ...
        print(f"✗ Failed to setup planner context: {e}")
        sys.exit(1)
    ...
    for model_tier in model_tiers:
        ...
        for i, scenario in enumerate(scenarios, 1):
            ...
            try:
                ...
                result = test_planner_with_scenario(scenario, context, model_tier)
    ...
    generator = ReportGenerator("planner_multi_model", results_dir=results_dir)
    ...
    try:
        ...
        logger.error("test_planner.report_generation.failed", error=str(e))
