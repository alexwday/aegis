"""
Specialized PDF report generation for Router agent testing.
Adapted from the clarifier's improved design with router-specific formatting.
"""
...
class RouterPDFGenerator:
    """Generate PDF reports for router agent testing with improved formatting."""
    ...
    def generate(self, metadata: Dict[str, Any], results: List, summary: Dict[str, Any]) -> Path:
        """Generate the router-specific PDF report."""
        ...
        # Title and metadata
        story.append(Paragraph(metadata.get("name", "Router Performance Test Report"), title_style))
        story.append(Paragraph("Multi-Model Routing Analysis", heading3_style))
        ...
        if "model_comparison" in metadata:
            story.append(Paragraph("Routing Accuracy by Model", heading3_style))
        ...
        # 3. DETAILED TEST RESULTS - Router-specific formatting
        ...
        if results:
            ...
            for scenario_num, (scenario_id, scenario_results) in enumerate(scenarios_by_id.items(), 1):
                ...
                if first_result.expected_output:
                    expected_route = first_result.expected_output
                    if isinstance(expected_route, dict) and "route" in expected_route:
                        expected_route = expected_route["route"]
                    expected_route_display = expected_route.replace("_", " ").title()
                    story.append(Paragraph(f"<b>Expected Route:</b> {expected_route_display}", small_style))
                
                story.append(Spacer(1, 6))
                
                # Results table for each model - simplified for router (no confidence)
                result_data = [["Model", "Status", "Route Decision", "Latency", "Cost"]]
                
                for r in scenario_results:
                    # Use the model tier directly (should be "small", "medium", or "large")
                    model_tier = (r.model_used or "default").upper()
                    status = "✓ Pass" if r.success else "✗ Fail"
                    
                    # Extract route decision
                    route_decision = r.actual_output
                    if isinstance(route_decision, dict):
                        route_decision = route_decision.get("route", route_decision.get("decision", "-"))
                    
                    # Format route for display
                    if route_decision and isinstance(route_decision, str):
                        route_display = route_decision.replace("_", " ").title()
                    else:
                        route_display = "-"
                ...
                # Create results table with router-appropriate column widths (no confidence column)
                result_table = Table(
                    result_data,
                    colWidths=[60, 60, 140, 60, 70]
                )
                ...
                table_style = [
                    ...
                    ("FONTSIZE", (0, 0), (-1, 0), 9),
                    ("ALIGN", (0, 0), (-1, -1), "LEFT"),
                    ("ALIGN", (3, 0), (5, -1), "CENTER"),  # Center align confidence, latency, cost
                    # Data
                    ("FONTSIZE", (0, 1), (-1, -1), 8),
                    ("GRID", (0, 0), (-1, -1), 0.5, colors.grey),
                    ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
        ...
        if failed_tests:
            ...
            for scenario_name, failures in failed_by_scenario.items():
                ...
                for r in failures:
                    ...
                    # Show routing mismatch
                    expected_route = r.expected_output
                    if isinstance(expected_route, dict) and "route" in expected_route:
                        expected_route = expected_route["route"]
                    
                    actual_route = r.actual_output
                    if isinstance(actual_route, dict):
                        actual_route = actual_route.get("route", actual_route.get("decision", str(actual_route)))
                    
                    expected_display = expected_route.replace("_", " ").title() if expected_route else "Unknown"
                    actual_display = actual_route.replace("_", " ").title() if actual_route else "Unknown"
                    ...
                    # Add confidence if available for debugging
                    if isinstance(r.actual_output, dict) and "confidence" in r.actual_output:
                        conf_val = r.actual_output.get("confidence")
                        if conf_val is not None:
                            story.append(Paragraph(f"<b>Confidence:</b> {conf_val:.1f}%", small_style))
