"""
RTS Subagent - Educational Placeholder Implementation
...
The main workflow (src/aegis/model/main.py) calls this when "rts" is needed:

    from .main import rts_agent

    for chunk in rts_agent(
...
async def rts_agent(
    ...
    """
    RTS subagent - generates placeholder rts data.
    ...
    Parameters from Aegis:
        ...
        database_id: Always "rts" for this subagent
    ...
    try:
        ...
        # ==================================================
        # This loads from prompts/rts/rts.yaml
        # and automatically includes fiscal, project, and restrictions
        try:
            system_prompt = load_subagent_prompt("rts")
            logger.debug(
            ...
            system_prompt = """You are the RTS subagent for Aegis.
            Generate realistic placeholder rts data based on the context provided."""

        # ==================================================
        # STEP 4: Build the user prompt with context
        # ==================================================
        user_prompt = f"""Generate rts data for this request:
...
Create realistic placeholder rts data that would answer this query.
Include specific details appropriate for rts data.
End with: *[RTS placeholder data - test mode]*"""
        ...
        add_monitor_entry(
            ...
            decision_details=f"Generated rts data for {len(bank_period_combinations)} bank-period combinations",
            custom_metadata={
        ...
        yield {
            "type": "subagent",
            "name": database_id,
            "content": f"\n⚠️ Error in RTS subagent: {error_msg}\n",
        }


# NOTES FOR REPLACING THIS IMPLEMENTATION:
#
# 1. Keep the function name: rts_agent
