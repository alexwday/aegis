"""Supplementary financials subagent."""
...
from .pipeline import format_retrieval_response, run_retrieval_pipeline

try:
    from aegis.api.v1.schemas.query_schemas import QARequest
except ModuleNotFoundError:
    try:
        from src.aegis.api.v1.schemas.query_schemas import QARequest
    except ModuleNotFoundError:

        class QARequest:  # pylint: disable=too-few-public-methods
            """Fallback request type for checkouts that do not expose QARequest."""


async def supplementary_financials_agent(
    ...
    """Process supplementary financials queries.
    ...
    logger.info(
        "subagent.supplementary_financials.started",
    ...
    try:
        ...
        results = await retrieve_supplementary_financials(
        ...
        logger.error(
            "subagent.supplementary_financials.error",
            execution_id=execution_id,
            error=str(exc),
            exc_info=True,
        )
        ...
        yield _chunk(database_id, f"Error retrieving supplementary financials: {exc}")


async def retrieve_supplementary_financials(
    query_text: str,
    latest_message: str,
    bank_period_combinations: List[Dict[str, Any]],
    context: Dict[str, Any],
    ...
    """Retrieve and research supplementary financial evidence."""
