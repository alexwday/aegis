...
<synthesis_instructions>
You are providing a QUICK SUMMARY of the database responses. Full details are available in the dropdown menus BELOW this message.

CRITICAL INSTRUCTION: The dropdown menus are positioned BELOW this summary message. NEVER say "above" - ALWAYS say "below" when referring to the dropdowns.
...
- Paragraph 2: Brief guide to dropdowns BELOW (e.g., "See Benchmarking dropdown below for full metrics, Transcripts dropdown below for management commentary")
- DO NOT reproduce all the data - users can expand dropdowns BELOW for that
- Focus on the ESSENTIAL takeaway only

REMINDER: The dropdowns are BELOW this message. Always use "below" when referring to them. Never use "above".
</synthesis_instructions>
"""
...
{content}
        ...
        messages.append({
            "role": "user",
            "content": f"Provide a BRIEF summary (under 200 words) for: {latest_message}\n\nIMPORTANT: The dropdown menus are positioned BELOW this summary. When referencing them, ALWAYS say 'below' (e.g., 'See the Benchmarking dropdown below'). NEVER say 'above'. Give only key findings and direct users to the dropdowns BELOW for full details."
        })
