...
def identify_gaps(
    ...
    if etl_filter:
        ...
        if ETL_CONFIGS[etl_filter].get("schedule_driven", False):
            ...
            return gaps
        if not ETL_CONFIGS[etl_filter].get("run_independently", True):
            logger.info(f"ETL {etl_filter} is generated by its parent ETL, skipping gap detection")
            return gaps
        etl_types = [etl_filter]
    else:
        etl_types = [
            name
            for name, cfg in ETL_CONFIGS.items()
            if not cfg.get("schedule_driven", False) and cfg.get("run_independently", True)
        ]
