...
def _format_categories_for_prompt(categories: Sequence[Dict[str, Any]]) -> str:
    ...
    for category in categories:
        ...
        category_group = _escape_for_prompt(_clean_text(category.get("category_group", "")))
        lines.append(f"    <group>{category_group}</group>")
        lines.append(
            "    <transcript_sections>"
...
def _format_qa_block_prompt_entry(entry: Dict[str, Any]) -> str:
    ...
    return (
        ...
        "</speaker_affiliation>\n"
        "  <participant_type>"
        f"{_escape_for_prompt(entry.get('participant_type', ''))}"
        "</participant_type>\n"
        "  <preview>\n"
        f"{entry.get('preview', '')}\n"
        "  </preview>\n"
...
def _render_md_block_prompt(block: Dict[str, Any]) -> str:
    ...
    return (
        ...
        "  <speaker_affiliation>"
        f"{_escape_for_prompt(block.get('speaker_affiliation', ''))}"
        "</speaker_affiliation>\n"
        "  <sentences>\n"
...
def _render_qa_turn_prompt(turn: Dict[str, Any]) -> str:
    ...
    return (
        ...
        "    <speaker_affiliation>"
        f"{_escape_for_prompt(turn.get('speaker_affiliation', ''))}"
        "</speaker_affiliation>\n"
        "    <sentences>\n"
        f"{chr(10).join(sentence_lines)}\n"
        "    </sentences>\n"
