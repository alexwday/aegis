"""
Transcript retrieval and formatting utilities for Call Summary ETL.
...
async def retrieve_full_section(
    ...
    try:
        async with get_connection() as conn:
            ...
            # Run diagnostics only when no results found
            if len(chunks) == 0:
                diagnostics = await get_filter_diagnostics(combo, context)
                logger.warning(
                    "etl.call_summary_editor.no_results_found",
            ...
            logger.info(
                "etl.call_summary_editor.full_section_retrieval",
                execution_id=execution_id,
        ...
        logger.error("etl.call_summary_editor.full_section_error", execution_id=execution_id, error=str(e))
        raise RuntimeError(
