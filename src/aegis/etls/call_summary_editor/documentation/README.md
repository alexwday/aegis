...
| **aegis_data_availability table** | PostgreSQL | Verifies transcript availability for the requested bank and period before expensive LLM work |
...
|-------|---------|-----------|--------|
| **1. Setup & Validation** | Validate inputs and prepare execution environment before expensive LLM operations | • `get_bank_info_from_config()`: Resolve bank by id/name/symbol<br>• `verify_and_get_availability()`: Confirm transcript availability for bank-period<br>• `load_categories_from_xlsx()`: Load Canadian/US category Excel based on bank type<br>• `setup_authentication()` + `setup_ssl()`: Prepare LLM auth and SSL config | Ensures valid bank-period combination exists and establishes secure API connections before NAS or LLM work begins |
| **2. Transcript Source Resolution** | Resolve and download the best raw transcript XML for the requested bank/period | • `get_nas_connection()`: Open SMB session<br>• `find_transcript_xml()`: Build NAS path from year/quarter/type/`path_safe_name`, choose best filename version<br>• `nas_download_file()`: Load XML bytes | Produces the authoritative raw transcript source for downstream parsing |
