...
async def retrieve_full_section(
    ...
    logger.info(
        "etl.call_summary.filter_diagnostics",
    ...
    try:
        async with get_connection() as conn:
            ...
            if len(chunks) == 0 and diagnostics.get("matching_all_filters", 0) == 0:
                logger.warning(
                    "etl.call_summary.no_results_found",
                    execution_id=execution_id,
            ...
            logger.info(
                "etl.call_summary.full_section_retrieval",
                execution_id=execution_id,
        ...
        logger.error(
            "etl.call_summary.full_section_error", execution_id=execution_id, error=str(e)
        )
        return []
