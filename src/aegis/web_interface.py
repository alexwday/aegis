            ...
            try:
                for chunk in model(model_input, **model_kwargs):
                    ...
                    if msg_type == "agent":
                        ...
                        yield f"data: {json.dumps({'type': 'agent', 'name': 'aegis', 'content': content})}\n\n"
                    
                    elif msg_type == "subagent":
                        # Subagent response - track separately
                        if msg_name not in subagent_responses:
                            subagent_responses[msg_name] = ""
                            # Signal new subagent stream starting
                            yield f"data: {json.dumps({'type': 'subagent_start', 'name': msg_name})}\n\n"
                        
                        subagent_responses[msg_name] += content
                        # Send as subagent chunk with name
                        yield f"data: {json.dumps({'type': 'subagent', 'name': msg_name, 'content': content})}\n\n"
